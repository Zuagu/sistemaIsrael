...
$("#filtro").keyup(function (e) {
    ...
    if (e.keyCode === 13) {
        if (_cuenta.length >= 6) {
            select_datos_cuenta_relacionada(_cuenta);
        } else {
            alert("El Parametro de la cuenta es muy corto");
        }
    }
});
...
// Funciones para agrupar gesiones
$("#tabs-swipe-demo .tab a").click( function () {
    let id_tab_click =  $(this).attr("id");
    if (id_tab_click === 'tab_gestiones') {
        $("#grup_gestiones").removeClass("hide");
    } else {
        $("#grup_gestiones").addClass("hide");
    }
    
});

$("#ordenar_contacto").click(function () {
    $("#ordenar_contacto").addClass("hide");
    $("#ordenar_normal").removeClass("hide");
    let cuenta = $("#cuenta_deudor").val();
    let inicio =$("#inicio_deudor").val().split(' ');
    select_gestiones_alto_contacto_cuenta(cuenta,inicio[0],'tbody_tabla_gestiones');
    
    
});

$("#ordenar_normal").click(function () {
    $("#ordenar_normal").addClass("hide");
    $("#ordenar_contacto").removeClass("hide");
      let cuenta = $("#cuenta_deudor").val();
    let inicio =$("#inicio_deudor").val().split(' ');
    select_gestiones_cuenta(cuenta, inicio[0],'tbody_tabla_gestiones');
...
$("#codigo_llamada").change(function(){
    let id_codigo = $("#codigo_llamada").val();
    let txt_actual_g = $("#numero_marcado_deudor").val();
    let cadena_texto = [
        /*0*/'',
        /*1*/'',
        /*2*/'',
        /*3*/'',
        /*4*/'Contacto sin éxito',
        /*5*/'',
        /*6*/'',
        /*7*/'',
        /*8*/'Cuenta liquidada sale de gestion',
        /*9*/'Sale de cobranza cuenta para Reactivar',
        /*10*/'Gestion por medio adicionales',
        /*11*/'Se deja mensaje en Buzon',
        /*12*/'Telefono no exite o no enlaza',
        /*13*/'Telefono suspendido',
        /*14*/'',
        /*15*/'',
        /*16*/'Se validan todos los numeros contacto sin exito',
        /*17*/'',
        /*18*/'',
        /*19*/'Sin contacto sin medios adicionales',
        /*20*/'',
        /*21*/''
    ];
    $("#gestion").val(txt_actual_g + ' ' + cadena_texto[parseInt(id_codigo)]);
});
//funcion de buscador
function buscar_cuentas_relacionadas_gestor(_busqueda, _id_puesto, _div) {
    $(".cargando").empty();
    $(".cargando").append(`<div class="progress"><div class="indeterminate"></div></div>`);
    var params = {
        action: "select_buscar_cuentas",
        busqueda: _busqueda,
        id_puesto: _id_puesto
    ...
    $.ajax({
        ...
        success: function (cuentas) {
            ...
            for (let item of cuentas) {
                $("#" + _div).append(`<tr class="b_cuenta_relacionada">
                <td class="c_relacionada">${item.cuenta}</td>
                <td>${item.nombre}</td>
                <td>${item.asignacion}</td>
                <td>${item.ultimo_estatus_cuenta}</td>
                <tr>`);
            }
            $(".cargando").empty();
        }
...
$("#buqueda_relacionada").keyup(function (e) {
    ...
    if (e.keyCode === 13) {
        if ($("#buqueda_relacionada").val().length > 5) {
            buscar_cuentas_relacionadas_gestor($("#buqueda_relacionada").val(), id_puesto_usuario, "tb_cont_busqueda");
...
$("#tb_cont_busqueda").on("click", ".b_cuenta_relacionada", function () {
    let cuenta = $(".c_relacionada", this).text();
    select_datos_cuenta_relacionada(cuenta);
    $("#modal_busqueda").modal("close");
    $("#buqueda_relacionada").val("");
...
$("#actualizar_telefonos").click(function () {
    update_telefonos_adicionales();
});
...
function select_datos_cuenta_relacionada(_cuenta) {
    ...
    $.ajax({
        ...
        success: function (datos_cuenta) {
            console.log(datos_cuenta);
            if (Object.keys(datos_cuenta).length !== 0) {
                for (var dato in datos_cuenta) {
                    $("#" + dato).empty();
                    $("#" + dato).val(datos_cuenta[dato]);
                }
                ...
                alert("No existe una cuenta con los parametros agregados");
            }
        }
    });
}
...
$("#cuenta_agenda_datos").click(function () {
    ...
    select_datos_cuenta_relacionada(cliente_unico);
    let id_folio_agenda = $("#folio_agenda_act").val();
    update_estatus_agenda(id_folio_agenda);
});
...
function update_telefonos_adicionales() {
    let params = {
        action: 'update_t_adicional',
        t1: $("#t_adicional_1").val(),
        t2: $("#t_adicional_2").val(),
        t3: $("#t_adicional_3").val(),
        cuenta: $("#cuenta_deudor").val()
    };
    console.log(params);
    $.ajax({
...
function select_gestiones_alto_contacto_cuenta(_cuenta, _fecha_inico, _div) {
    $("#" + _div).empty();
    $("#" + _div).append("<img src='image/preloader_lineal.gif' width='40%'>");
    var params = {
        action: "select_gestiones_alto_contacto_cuenta",
        cuenta: _cuenta,
        fecha_inico: _fecha_inico
    };
//    console.log(params);
