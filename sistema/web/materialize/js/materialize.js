...
var cont = 0;
var conta = 0;
$(".renglon").live("click", function (e) {
    if (e.ctrlKey) {
        if(conta == 0){$(".renglon").removeClass("renglon_selected");}
        if($(this).closest("tr").hasClass( "varios" )){
            $(this).closest("tr").removeClass("renglon_selected");
            $(this).closest("tr").removeClass("varios");
        }else{
            $(this).closest("tr").addClass("renglon_selected");
            $(this).closest("tr").addClass("varios");
            cont = conta + 1;
        }
    } else {
        $(".renglon").removeClass("renglon_selected");
        $(".renglon").removeClass("varios");
        $(this).closest("tr").addClass("renglon_selected");
        cont = 0;
    }
    conta = cont;
...
(function (factory) {
    ...
    function DatePicker(picker, settings) {
        ...
        // If there’s no value, default to highlighting “Hoy”.
        else {
    ...
    DatePicker.prototype.create = function (type, value, options) {
        ...
        // that it’s a valid date – otherwise default to Hoy.
    ...
    DatePicker.prototype.measure = function (type, value /*, options*/) {
        ...
        // If it's an integer, get a date relative to Hoy.
        ...
        return _.node(
                ...
                _.node('div', _.node('div', _.node('div', (settings.selectYears ? createMonthLabel() + createYearLabel() : createMonthLabel() + createYearLabel()) + createMonthNav() + createMonthNav(1), settings.klass.header) + _.node('table', tableHead + _.node('tbody', _.group({
                    ...
                    readonly: true
                        ...
                        _.node('div', _.node('button', settings.Hoy, "btn-flat picker__Hoy waves-effect", 'type=button data-pick=' + nowObject.pick + (isOpen && !calendar.disabled(nowObject) ? '' : ' disabled') + ' ' + _.ariaAttr({controls: calendar.$node[0].id})) + _.node('button', settings.clear, "btn-flat picker__clear waves-effect", 'type=button data-clear=1' + (isOpen ? '' : ' disabled') + ' ' + _.ariaAttr({controls: calendar.$node[0].id})) + _.node('button', settings.close, "btn-flat picker__close waves-effect", 'type=button data-close=true ' + (isOpen ? '' : ' disabled') + ' ' + _.ariaAttr({controls: calendar.$node[0].id})), settings.klass.footer), 'picker__container__wrapper'); //endreturn
